...
int test_msq_player(void){
  ...
  if (pid == 0){
	...
	t_myMsgbuf msgbuf;
	if (recv_msg(player.qid,  &msgbuf, BROADCAST) == -1){
		perror("msq :recv error");
		return (1);
	}
	printf("msgbug value is mytype = %ld x = %d y = %d kind = %d team_no = %d msg_order = %d\n", msgbuf.mytype, msgbuf.x, msgbuf.y, msgbuf.kind, msgbuf.team_no, msgbuf.msg_order);
    clear_ipcs(player.mid, player.sid, player.qid);
    return (0);
  }
}
